...
public class PRGen extends java.util.Random {
    ...
    public PRGen(byte[] key) {
        ...
        state = key; // used for initial setup, overwritten later
    ...
    protected int next(int bits) {
        ...
        int finalOutput = 0; // higher order should be 0
        byte[] counter = {0};
        for (int i = 0; i < bits; i++) {
            PRF f = new PRF(state); // Seed new PRF w/ previous state

            byte[] output = f.eval(counter); // output PRF call
           
            Array.setByte(counter, 0, (byte)(i+1)); // change counter to generate new state
            state = f.eval(counter);
            
            finalOutput = finalOutput ^ ((output[0] << 31) >>> (31 - i)); // shift bit to correct position & update final output
        }
        return finalOutput;
    }
}
