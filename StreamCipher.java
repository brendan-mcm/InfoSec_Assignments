...
public class StreamCipher {
    ...
    // Instance variables.
    PRGen mainGen; // reference to PRGen
    ...
    public StreamCipher(byte[] key, byte[] nonceArr, int nonceOffset) {
        ...
        // Seed PRF with given key, create session key w/ nonce using that PRF
        PRF seshPRF = new PRF(key);
        byte[] sessionKey = seshPRF.eval(nonceArr, nonceOffset, NONCE_SIZE_BYTES);
        mainGen = new PRGen(sessionKey); // Seed PRGen with session key
    }
    ...
    public byte cryptByte(byte in) {
        byte nextByte = (byte) mainGen.next(8); // returns pseudorandom byte
        return (byte) (nextByte ^ in); // XOR to provide symmetric encryption
    }
    ...
    public void cryptBytes(byte[]  inBuf, int  inOffset, 
        ...
        // Iterates over specified bytes and updates the output buffer
        for (int i = 0; i < numBytes; i++) {
            byte crypted = cryptByte(inBuf[inOffset+i]);
            Array.setByte(outBuf, outOffset+i, crypted);
        }
    }
}
